...
eliminate these operations by using the following
...
square roots can be eliminated by multiplying by $r_1^\pm-r_2^\mp$,
bringing us to~$\sign(a_1 a_2)\sign(a_1 a_2
(r_1^\pm-r_2^\mp))\sign(a_1 a_2 (r_1^\pm - r_2^\pm) (r_1^\pm -
r_2^\mp))$.  This is sufficient to compute the correct result, as the
simplified equation contains only one square root, multiplications,
and additions.  When simplified, the final sign is computed from
$2a_2^2b_1^2-4a_1a_2^2c_1+4a_1^2a_2c_2-2a_1a_2b_1b_2\pm 2
...
$a_1-b_1$.  The sign of $a_1-b_1$ can then be computed as shown in
