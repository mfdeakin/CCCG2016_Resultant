...
\Michael{I'm worried my notation is confusing here. The $\pm$ and
  ...
  the manipulations}\Jack{That didn't bother me.}
...
\Michael{This isn't what I wanted to convey, as this method will work
  regardless of the sides they're on...}\Jack{To find out which of $r_2^\pm$ is close to $r_1$,  compare $r_1$ with $-b^2/a^2$. You learn the sign of the difference with the other root as well.  E.g., if $r_1>-b^2/a_2$ then we know $r_1>r_2^-$ and want to compare $r_1?r_2^+$.  Do I need to spell that out? What method do you refer to?}
...
we can learn this from a lower precision test against $-b^2/a^2$, since $r_2^- \le -b^2/a^2 \le r_2^+$.
...
that computed the line-quadric intersection order of 100k random
lines with a scene of quadric surfaces.  MPFR\Jack{cite or url footnote} was used to implement
the increased precision when required.

\Jack{Describe the data first.}
...
line.  The sign of the discriminant determines whether the intersection
is to be considered for sorting later.  
This does not guarantee
correctness\Jack{This is the wrong way to say this, because you haven't stated the guarantee.} in cases where the discriminant is close to zero, but this
is okay because we are guaranteeing to sort intersections
...
\Jack{You make this too complicated.  We made a choice.  State the choice first, then the advantages of the choice.  E.g., Our test scenes are composed of ellipsoids, because then every line intersects an ellipsoid in zero or two points, giving a good quadratic.  
On the other hand, you don't say how quadrics are represented -- how many bits?  Do you use exact inputs or not? Or do you just start with the quadratic?  How many bits for coefficients?  What is the exponent range?  Are the examples that fail well scaled?}
...
quadric surfaces allowed in the test scenes.  Any scene that is used
...
\Jack{I need enough detail that I can replicate your experiment.}
...
at~$x_0=0.5, y_0=0.5, z_0=0.5$ units with a radius of~$R_0=0.5$
...
\subsection{Hardware}\Jack{Part of setup}
