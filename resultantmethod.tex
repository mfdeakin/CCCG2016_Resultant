...
First, it is clear that increasing the precision of a computation is
...
are needed to find lines which cause errors in the Packed Spheres
scene, but we can expect several to occur by the
...
Spheres scenes, the constant term appears to mostly be noise,
...
\begin{figure}
  ...
  \caption{Evaluation Time for a Line (ms) vs. the Number of
    ...
    Nested Spheres test scenes on the Gentoo machine; {\bf Circles:
      Approximation Method at Input Precision}; {\bf X's:
...
We have assumed that we know the order of all roots except one pair. Even if one's application does not provide this information, for    
 quadratic equations it is relatively easy to obtain using lower precision than it takes to compare roots.  The zero of the derivative $x_i=-b_i/a_i$ separates $r_i^-$ and $r_i^+$ by twice the discriminant.  If $x_1=x_2$ then comparing squared discriminants tells us all we need to know about  root orders.  When, wlog, $x_1<x_2$, we use the signs of both  quadratics at $x_1$ and $x_2$ to bound roots to intervals, and can again compare squared discriminants to reveal the order for all but one pair.  
